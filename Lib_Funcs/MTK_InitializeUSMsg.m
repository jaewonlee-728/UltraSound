%> ========================================================================
%> @function 
%>       varargout = MTK_InitializeUSMsg(varargin)
%>
%> @brief 
%>       Initialize USMessage of OpenIGTLinkMUSiiC
%>
%> @Sub-functions:
...
%>
%>       - USMsg (USMessage)
%>         : USMessage: Matlab version of USMessage of OpenIGTLinkMUSiiC
%>
%>       - Number of outputs
%>          - 0 or 1 output 
%>               >> USMsg
...
function varargout = MTK_InitializeUSMsg(varargin)
    ...
    %% Update variables with input data set
     while i <= nVarargins
        if isnumeric (varargin{i})
            ...
            if strcmpi('M_debug', varargin{i}) == 1
                i                   = i+1;
                bDebug              = varargin{i};
            elseif strcmpi('M_datatype', varargin{i}) == 1
                i                   = i+1;
                iDataType           = varargin{i};
            elseif strcmpi('M_scalartype', varargin{i}) == 1
                i                   = i+1;
                iScalarType         = varargin{i};
            elseif strcmpi('M_endian', varargin{i}) == 1
                i                   = i+1;
                iEndian             = varargin{i};
            elseif strcmpi('M_coordinate', varargin{i}) == 1
                i                   = i+1;
                iCoordinate         = varargin{i};
            end 
        end
        i                = i+1;
     end
     ...
     % 1: Scalar
     % 2: Vector
     USMsg.Body.h.type         = int32(iDataType);
     ...
     % 10: float32 (matlab:single)
     % 11: float64 (matlab:double)
     USMsg.Body.h.scalarType   = int32(iScalarType);

     % Endian for Image Data
     % 1: BIG Endian
     % 2: Little Endian
     USMsg.Body.h.endian       = int32(iEndian);

     % Coordinate System of Image Data
     % 1: RAS
     % 2: LPS
     USMsg.Body.h.coordinate   = int32(iCoordinate);
     ...
     USMsg.Body.uh             = MTK_InitializeUSMsgHeader(varargin{:});
